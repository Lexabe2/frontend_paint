    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-600 font-medium">Загрузка потоков...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-6xl mx-auto px-6 py-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl font-bold text-slate-900">Потоки</h1>
            </div>
            ...
            <div className="flex items-center gap-2">
              {pattern && (
                <a
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-all"
                >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all"
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-6">
        {filteredFlows.length === 0 && searchQuery && (
          <div className="bg-white border border-slate-200 rounded-lg p-8 text-center">
            <p className="text-slate-500">Потоки не найдены</p>
          </div>
        )}
        ...
        {filteredFlows.length === 0 && !searchQuery && flows.length === 0 && (
          <div className="bg-white border border-slate-200 rounded-lg p-12 text-center">
            ...
            <p className="text-slate-500 mb-4">Нет доступных потоков</p>
            <button
              onClick={() => setShowModal(true)}
              className="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-all"
            >
              <Upload className="w-4 h-4" />
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      {completed && (
                        <div className="flex items-center gap-1.5 px-2 py-0.5 bg-green-50 text-green-700 rounded text-xs font-medium flex-shrink-0">
                    ...
                    <div className="flex items-center gap-4 flex-shrink-0">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xl font-bold text-slate-900">{flow.total}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 max-h-0 overflow-hidden opacity-0 group-hover:max-h-96 group-hover:opacity-100 transition-all duration-300">
                    <div className="pt-3 border-t border-slate-100 space-y-2">
                      ...
                      <div className="mt-3 pt-2 border-t border-slate-100 text-center">
                        <p className="text-xs text-slate-500 italic">Нажмите, чтобы подробно посмотреть поток</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
              <h2 className="text-lg font-semibold text-slate-900">Загрузить Excel-файл</h2>
              <button
                onClick={() => {
                  setShowModal(false);
                  setFile(null);
            ...
            <div className="flex items-center gap-3 px-6 py-4 bg-slate-50 border-t border-slate-200">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-medium"
              >
                {uploading ? (
                  <>
              ...
              <button
                ...
                disabled={uploading}
                className="px-4 py-2 text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 disabled:opacity-50 transition-all font-medium"
              >
                Отмена
              </button>
            </div>
