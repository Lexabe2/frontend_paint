...
export default function ResponsiveHeader() {
    ...
    // Функция для фильтрации элементов по группам
    const getFilteredItemsForGroup = (groupKey, items) => {
        // Если это главная группа, показываем только главную страницу
        if (groupKey === 'main') {
            return items.filter(item => item.id === 'dashboard' || item.path === '/')
        }

        // Для группы склада показываем все связанное со складом
        if (groupKey === 'warehouse' || groupKey === 'group1') {
            return items.filter(item => {
                const warehouseKeywords = ['склад', 'warehouse', 'заявк', 'request', 'приемк', 'receive']
                const itemText = `${item.label} ${item.path} ${item.id}`.toLowerCase()
                return warehouseKeywords.some(keyword => itemText.includes(keyword)) ||
                       item.category === 'warehouse'
            })
        }

        // Для группы покраски показываем все связанное с покраской
        if (groupKey === 'painting' || groupKey === 'group2') {
            return items.filter(item => {
                const paintingKeywords = ['покрас', 'paint', 'краск', 'цвет', 'color', 'подготовк', 'prepare']
                const itemText = `${item.label} ${item.path} ${item.id}`.toLowerCase()
                return paintingKeywords.some(keyword => itemText.includes(keyword)) ||
                       item.category === 'painting'
            })
        }

        // По умолчанию возвращаем все элементы
        return items
    }
    ...
    const getAvailableGroups = () => {
        ...
        Object.entries(navigationGroups).forEach(([groupKey, group]) => {
            ...
            // Затем фильтруем по группам
            const filteredItems = getFilteredItemsForGroup(groupKey, permissionFilteredItems)
    ...
    const getMainNavItem = () => {
        ...
        return currentGroup?.items.find(item => item.id === 'dashboard') || currentGroup?.items[0]
    }
    ...
    if (!user) {
        return (
            <div className="fixed top-0 left-0 right-0 z-50 h-16 bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center">
                ...
                {!isMobile && (
                    <>
                        ...
                        <div className="absolute inset-0 overflow-hidden pointer-events-none">
                            <div className="absolute top-2 left-1/4 w-1 h-1 bg-cyan-400 rounded-full animate-ping opacity-40"></div>
                            <div className="absolute top-4 right-1/3 w-1 h-1 bg-purple-400 rounded-full animate-ping opacity-30" style={{animationDelay: '1s'}}></div>
                            <div className="absolute top-3 left-2/3 w-1 h-1 bg-blue-400 rounded-full animate-ping opacity-50" style={{animationDelay: '2s'}}></div>
                        </div>
                    </>
                )}
                    ...
                    <div className={`flex items-center justify-between ${isMobile ? "" : "h-16"}`}>
                        ...
                        <div className="flex items-center space-x-3">
                            <motion.div
                                ...
                                {!isMobile && (
                                    <>
                                        <div className="absolute -inset-1 bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 rounded-2xl blur opacity-40 group-hover:opacity-70 transition-opacity duration-300"></div>
                                        <div className="absolute inset-0 bg-gradient-to-r from-cyan-400/20 to-purple-600/20 rounded-2xl animate-pulse"></div>
                                    </>
                                )}
                            </motion.div>
                            ...
                            <div>
                                ...
                                {!isMobile && (
                                    <div className="text-xs text-gray-400 font-medium">Management System</div>
                                )}
                            </div>
                            ...
                            {!isMobile && (
                                <div className="hidden lg:flex items-center space-x-3 ml-8">
                                    <div className="flex items-center space-x-2 px-3 py-1.5 bg-gray-800/50 backdrop-blur-sm rounded-lg border border-gray-700/50">
                                ...
                                <div className="relative navigation-dropdown" ref={navigationRef}>
                                    ...
                                    <AnimatePresence>
                                        {isNavigationOpen && (
                                            ...
                                            >
                                                {/* Animated background */}
                                                <div className="absolute inset-0 bg-gradient-to-br from-gray-800/50 via-gray-900/50 to-gray-800/50 animate-pulse opacity-50"></div>
                                ...
                                <AnimatePresence>
                                    {isProfileOpen && (
                                        ...
                                        >
                                            {/* Animated background */}
                                            {!isMobile && (
                                                <div className="absolute inset-0 bg-gradient-to-br from-gray-800/50 via-gray-900/50 to-gray-800/50 animate-pulse opacity-30"></div>
                                            )}
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="flex items-center space-x-2 mt-1">
                                                                ...
                                                                <span className={`text-xs ${
                                                                    ...
                                                                    <div className="w-1.5 h-1.5 bg-green-400 rounded-full mr-1 animate-pulse shadow-sm shadow-green-400/50"></div>
                                                                    Онлайн
                                                                </span>
                                                            </div>
                                                        </div>
                ...
                {isMobileMenuOpen && (
                    <>
                        ...
                        >
                            ...
                            {!isMobile && (
                                <div className="absolute inset-0 bg-gradient-to-br from-gray-900/80 via-gray-800/80 to-gray-900/80 animate-pulse opacity-50"></div>
                            )}
